...
-(void)setRunOnStartup:(BOOL)val {
    if (@available(macOS 13.0, *)) {
        ...
        CFStringRef appId = (__bridge CFStringRef)@"com.hqdvn.MyOpenKeyHelper";
...
-(void)onConvertTool {
    if (_convertWC == nil) {
        ...
        _convertWC.window.collectionBehavior = NSWindowCollectionBehaviorMoveToActiveSpace | NSWindowCollectionBehaviorParticipatesInCycle;
    }
    if ([_convertWC.window isMiniaturized]) {
        [_convertWC.window deminiaturize:nil];
    }
    [NSApp activateIgnoringOtherApps:YES];
    [_convertWC.window makeKeyAndOrderFront:nil];
    [_convertWC.window orderFrontRegardless];
}
...
-(void) onControlPanelSelected {
    if (_mainWC == nil) {
        ...
        _mainWC.window.collectionBehavior = NSWindowCollectionBehaviorMoveToActiveSpace | NSWindowCollectionBehaviorParticipatesInCycle;
    }
    if ([_mainWC.window isMiniaturized]) {
        [_mainWC.window deminiaturize:nil];
    }
    [NSApp activateIgnoringOtherApps:YES];
    [_mainWC.window makeKeyAndOrderFront:nil];
    [_mainWC.window orderFrontRegardless];
}

-(void) onMacroSelected {
    if (_macroWC == nil) {
        _macroWC = [[NSStoryboard storyboardWithName:@"Main" bundle:nil] instantiateControllerWithIdentifier:@"MacroWindow"];
        _macroWC.window.collectionBehavior = NSWindowCollectionBehaviorMoveToActiveSpace | NSWindowCollectionBehaviorParticipatesInCycle;
    }
    if ([_macroWC.window isMiniaturized]) {
        [_macroWC.window deminiaturize:nil];
    }
    [NSApp activateIgnoringOtherApps:YES];
